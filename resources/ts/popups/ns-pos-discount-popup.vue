    ...
    <div id="discount-popup" class="ns-box shadow w-[85.71vw] md:w-[60vw] lg:w-[60vw] xl:w-[40vw] relative">
        <div class="flex-shrink-0 flex justify-between items-center p-2 border-b ns-box-header">
            <div>
                ...
                <h1 class="text-xl font-bold text-fontcolor text-center" v-if="type === 'cart'">{{ __( 'Cart Discount' ) }}</h1>
            </div>
            <div>
        ...
        <div id="switch-mode" class="flex">
            <button v-if="! popup.params.reference.disable_flat" @click="setPercentageType('flat')" :class="mode === 'flat' ? 'bg-tab-active' : 'bg-tab-inactive text-secondary'" class="outline-hidden w-1/2 py-2 flex items-center justify-center">{{ __( 'Flat' ) }}</button>
